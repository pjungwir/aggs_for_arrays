...
assign sql_rows <<EOQ
  ...
  SELECT  SUM((value - mean)^3 / n) / stddev^3
  FROM    samples,
          stats
  WHERE   measurement_id = (SELECT MIN(id) FROM measurements)
...
assign sql_array <<EOQ
  ...
  SELECT  SUM((v - mean)^3 / n) / stddev^3
  FROM    (SELECT UNNEST(values) v
           FROM   sample_groups
           WHERE   measurement_id = (SELECT MIN(id) FROM measurements)
          ) x,
...
query >/dev/null <<'EOQ'
CREATE OR REPLACE FUNCTION skewness_from_array (vals FLOAT[]) RETURNS FLOAT AS $$
DECLARE
BEGIN
  RETURN (
  WITH stats AS (
...
assign plpgsql_array <<EOQ
  SELECT  skewness_from_array(values)
...
assign c_array <<EOQ
  SELECT  array_to_skewness(values)
  FROM    sample_groups s
  WHERE   s.measurement_id = (SELECT MIN(id) FROM measurements)
  ;
EOQ
