...
Datum array_to_skewness(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(array_to_skewness);

/**
 * Returns the skewness from an array of numbers.
 * by Paul A. Jungwirth
 */
Datum
array_to_skewness(PG_FUNCTION_ARGS)
{
  // Our arguments:
  ArrayType *vals;
  ...
  if (valsType != INT2OID &&
      ...
      valsType != FLOAT4OID &&
      valsType != FLOAT8OID) {
    ereport(ERROR, (errmsg("Skewness subject must be SMALLINT, INTEGER, BIGINT, REAL, or DOUBLE PRECISION values")));
  }
...
&valsContent, &valsNullFlags, &valsLength);
  ...
  switch (valsType) {
    case INT2OID:
      ...
      for (i = 0; i < valsLength; i++) {
        ret += pow(DatumGetInt16(valsContent[i]) - mean, 3) / valsLength;
      }
      break;
    case INT4OID:
      for (i = 0; i < valsLength; i++) {
      ...
      stddev = sqrt(stddev / valsLength);
      for (i = 0; i < valsLength; i++) {
        ret += pow(DatumGetInt32(valsContent[i]) - mean, 3) / valsLength;
      }
      break;
    case INT8OID:
      for (i = 0; i < valsLength; i++) {
      ...
      stddev = sqrt(stddev / valsLength);
      for (i = 0; i < valsLength; i++) {
        ret += pow(DatumGetInt64(valsContent[i]) - mean, 3) / valsLength;
      }
      break;
    case FLOAT4OID:
      for (i = 0; i < valsLength; i++) {
      ...
      stddev = sqrt(stddev / valsLength);
      for (i = 0; i < valsLength; i++) {
        ret += pow(DatumGetFloat4(valsContent[i]) - mean, 3) / valsLength;
      }
      break;
    case FLOAT8OID:
      for (i = 0; i < valsLength; i++) {
      ...
      stddev = sqrt(stddev / valsLength);
      for (i = 0; i < valsLength; i++) {
        ret += pow(DatumGetFloat8(valsContent[i]) - mean, 3) / valsLength;
      }
      break;
    default:
      ereport(ERROR, (errmsg("Skewness subject must be SMALLINT, INTEGER, BIGINT, REAL, or DOUBLE PRECISION values")));
