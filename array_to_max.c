...
Datum array_to_max(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(array_to_max);

/**
 * Returns a max from an (unsorted) array of numbers.
 * by Paul A. Jungwirth
 */
Datum
array_to_max(PG_FUNCTION_ARGS)
{
  // Our arguments:
  ArrayType *vals;
  ...
  if (valsType != INT2OID &&
      ...
      valsType != FLOAT4OID &&
      valsType != FLOAT8OID) {
    ereport(ERROR, (errmsg("Max subject must be SMALLINT, INTEGER, BIGINT, REAL, or DOUBLE PRECISION values")));
  }
